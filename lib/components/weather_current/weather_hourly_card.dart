...
class HourlyWeatherCard extends StatefulWidget {
  HourlyWeatherCard({super.key, required this.hourly});
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: Card(
        ...
        child: Column(
          children: [
            const Text("Прогноз погоди на сьогодні",
                style: TextStyle(fontSize: 16, height: 1.75)),
            const Divider(),
            SingleChildScrollView(
              scrollDirection: Axis.horizontal,
              child: Row(
                  children: _HourlyTile.buildList(
                      _HourlyTile.tilesFromAPI(widget.hourly), context)),
            )
          ],
        ),
      )),
...
class _HourlyTile {
  ...
  static List<_HourlyTile> tilesFromAPI(Hourly hourly) {
    ...
    DateTime limit = DateTime.fromMillisecondsSinceEpoch(
