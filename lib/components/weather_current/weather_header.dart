...
class _WeatherHeader extends State<WeatherHeader> {
  ...
  TextStyle temperature =
      const TextStyle(fontSize: 52, fontWeight: FontWeight.w600);
  TextStyle updatedStyle =
      const TextStyle(fontSize: 11, fontWeight: FontWeight.w300, height: 0.2);
          ...
          children: [
            Column(
              ...
              children: [
                ...
                Text(
                    "Останнє оновлення ${formatter.format(widget.lastUpdate ?? DateTime.parse("1970-01-01"))}",
                    style: updatedStyle),
                ...
                Baseline(
                    ...
                    child: Text(
                        WeatherTranslator.getWeatherDescription(
                                widget.status) ??
                            "API Failed",
                        style: description)),
                Text(
                    "${widget.maxTemp.round()}° / ${widget.minTemp.round()}° відчувається як ${widget.perceivedTemp.round()}°")
              ],
            ),
            Container(
