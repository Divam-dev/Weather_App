...
class DailyWeatherCard extends StatefulWidget {
  DailyWeatherCard({super.key, required this.daily});
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: Card(
        ...
        child: Column(
          children: <Widget>[
                const Text("Прогноз погоди на 7 днів",
                    style: TextStyle(fontSize: 16, height: 1.75)),
                const Divider(),
              ] +
              _DailyTile.buildList(
                  _DailyTile.tilesFromAPI(widget.daily), context),
        ),
      )),
    );
  }
    ...
    for (int i = 0; i < daily.weatherCode.length; i++) {
      tiles.add(_DailyTile(
          ...
          precipitationProbability:
              (daily.precipitationProbability?[i] ?? 69) * 1.0,
          date: daily.dates[i]));
    }
