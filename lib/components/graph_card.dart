...
class GraphCard extends StatefulWidget {
  GraphCard(
    ...
    for (var list in graphLines) {
      for (var value in list.list) {
...
class _GraphCard extends State<GraphCard> {
  ...
  LineChartData buildData() {
    var (_, type) =
        GraphUtilities.getRangeSize(widget.graphStart, widget.graphEnd);

    return LineChartData(
      minX: 0,
      minY: widget.graphMin,
      maxX: (widget.graphLines[0].list.length /
              (widget.graphLines[0].list.length /
                  GraphUtilities.getXGap(widget.graphStart, widget.graphEnd)))
          .floorToDouble(),
      maxY: widget.graphMax,
      gridData: FlGridData(
          show: true,
          drawVerticalLine: true,
          horizontalInterval: (widget.graphMax < 50)
              ? 1
              : (widget.graphMax < 150)
                  ? 25
                  : (widget.graphMax < 1000)
                      ? 100
                      : 1000,
          verticalInterval:
              GraphUtilities.getXGap(widget.graphStart, widget.graphEnd) * 1.0,
          getDrawingHorizontalLine: (value) {
            return const FlLine(color: Colors.blueGrey, strokeWidth: 1);
          },
      ...
      titlesData: FlTitlesData(
          ...
          bottomTitles: AxisTitles(
              sideTitles: SideTitles(
                  showTitles: true,
                  interval: GraphUtilities.getXGap(
                          widget.graphStart, widget.graphEnd) *
                      1.0,
                  ...
                  getTitlesWidget: (value, meta) {
                    return RotatedBox(
                      quarterTurns: -3,
                      child: Text(
                          " ${GraphUtilities.getHorizontalLabel(widget.graphStart, widget.graphEnd, type, value)}",
                          style: graphLabel),
                    );
                  })),
  ...
  List<LineChartBarData> _buildChartData() {
    ...
    for (var line in widget.graphLines) {
  ...
  List<FlSpot> _buildLineSpots(GraphListComponent line) {
    ...
    double itemsPerGap = line.list.length /
        GraphUtilities.getXGap(widget.graphStart, widget.graphEnd);
    double itemOffset = 1 / itemsPerGap;

    for (var value in line.list) {
      spots.add(FlSpot(index, (value ?? 0) * 1.0));
    ...
    for (int i = 0; i < widget.graphLines.length; i++) {
      widgets.add(Padding(
          ...
          child: GestureDetector(
              onTap: () {
                setState(() {
                  widget.graphLines[i].ignoreInDraw =
                      !widget.graphLines[i].ignoreInDraw;
                });
              },
              child: Row(
                children: [
                  Icon(
                      (widget.graphLines[i].ignoreInDraw)
                          ...
                          : Icons.circle,
                      color: widget.graphLines[i].color,
                      size: 14),
                  const SizedBox(width: 8),
                  Text(widget.graphLines[i].title,
