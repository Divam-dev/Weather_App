...
class _CurrentWeather extends State<CurrentWeather> {
  ...
  bool useColoredBackgrounds = true;

  Future loadFromStorage() async {
    await PreferencesStorage.initialize();

    useColoredBackgrounds = await PreferencesStorage.readBoolean(
            SettingPreferences.SHOW_COLOR_BACKGROUND) ??
        true;

    if (await PreferencesStorage.readBoolean(
            SettingPreferences.USE_GPS_DEFAULT) ==
        true) {
    ...
    int? lastLoadTimeFromStorage =
        await PreferencesStorage.readInteger(PreferencesStorage.GEO_LAST_LOAD);
    if (lastLoadTimeFromStorage == null) return;
    String? city =
        await PreferencesStorage.readString(PreferencesStorage.GEO_CITY);
    String? fullName =
        await PreferencesStorage.readString(PreferencesStorage.GEO_FULLNAME);
    double? lat =
        await PreferencesStorage.readDouble(PreferencesStorage.GEO_LAT);
    double? lon =
        await PreferencesStorage.readDouble(PreferencesStorage.GEO_LON);

    setState(() {
      selectedGeo = Geo(lat ?? 0.0, lon ?? 0.0, city: city, fullName: fullName);
      lastWeatherUpdate =
          DateTime.fromMillisecondsSinceEpoch(lastLoadTimeFromStorage);
    });
  ...
  Future getWeatherForSelectedGeo() async {
    ...
    try {
      ...
      setState(() {
        ...
        if (useColoredBackgrounds) {
          backgroundGradient = LinearGradient(
              begin: Alignment.topCenter,
              end: Alignment.bottomCenter,
              colors: [
                WeatherTranslator.getWeatherColor(
                    currentWeather!.weatherCode, isDarkMode),
                backgroundColor!
              ]);
        }
      });
  ...
  Widget build(BuildContext context) {
    isDarkMode = (MediaQuery.of(context).platformBrightness == Brightness.light)
        ? false
        : true;
    backgroundColor = ((isDarkMode) ? ThemeData.dark() : ThemeData.light())
        .colorScheme
        .background;
                                ...
                                child: Column(
                                    ...
                                    children: [
                                      WeatherHeader(
                                          ...
                                          minTemp:
                                              dailyWeather!.minTemperature[0],
                                          maxTemp:
                                              dailyWeather!.maxTemperature[0],
                                          perceivedTemp: currentWeather!
                                              .apparentTemperature,
                                      ...
                                      SizedBox(
                                          ...
                                          child: GridView.count(
                                            ...
                                            children: [
                                              WindCard(
                                                  windDirection: currentWeather!
                                                      .windDirection,
                                                  windSpeed: currentWeather!
                                                      .windSpeed),
                                              SunsetSunriseCard(
                                                  sunrise: dailyWeather!
                                                          .sunrise?[0] ??
                                                      "1970-01-01 00:00",
                                                  sunset: dailyWeather!
                                                          .sunset?[0] ??
