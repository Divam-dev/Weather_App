...
class Geo {
  ...
  bool isEqual(Geo geo) => (geo.city == city &&
      geo.lat == lat &&
      geo.lon == lon &&
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      "coordinates": {"latitude": this.lat, "longitude": this.lon}
    };
  }
  ...
  static Future<Geo> getLocation() async {
    ...
    if (!serviceEnabled) {
      ...
      if (!serviceEnabled) {
        throw GeoException("Service unavailable", serviceEnabled: false);
      }
    }
    ...
    if (permissionStatus == PermissionStatus.denied) {
      ...
      if (permissionStatus != PermissionStatus.granted) {
        throw GeoException("Permission not granted", permission: false);
      }
    }
  ...
  static void addFavourite(Geo geo) {
    bool unique = true;
    for (var city in favouriteLocations) {
  ...
  StatefulBuilder toListItem(Function(Geo) weatherSetter,
      {Function()? triggerFavouriteListUpdate}) {
  ...
  static List<StatefulBuilder> buildFavouritesList(Function(Geo self) onTap) {
    ...
    for (var location in favouriteLocations) {
